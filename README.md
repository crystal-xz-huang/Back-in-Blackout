...
## Due: Week 5 Friday, 5pm [Sydney Local Time](https://www.timeanddate.com/worldclock/australia/sydney) (28th June 2024)

Key notes:

- **No submissions will be accepted greater than 7 days after the assignment due date. This includes students with special considerations and ELP.** See [this](#102-extensions).
- There will be no "Short Extensions" from Special Consideration for this assignment.
- Submission must be pushed onto the `main` branch on GitLab.
- Ensure your submission passes our dryrun [here](#71-dryrun-on-cse).

## Table Of Contents <!-- omit in toc -->

- [Assignment-i](#assignment-i)
  - [Due: Week 5 Friday, 5pm Sydney Local Time (28th June 2024)](#due-week-5-friday-5pm-sydney-local-time-28th-june-2024)
  - [Value: 15% of the Course Mark](#value-15-of-the-course-mark)
- [0. Change Log](#0-change-log)
- [1. Preamble and Problem](#1-preamble-and-problem)
  - [1.1 A simple example](#11-a-simple-example)
  - [1.2 Simulation](#12-simulation)
- [2. Requirements 🪐](#2-requirements-)
  - [2.1 Assumptions](#21-assumptions)
  - [2.2 Devices 🖨️](#22-devices-️)
  - [2.3 Files 📨](#23-files-)
  - [2.4 Satellites 🛰️](#24-satellites-️)
  - [2.5 Visualisation 🎨](#25-visualisation-)
- [3. Program Structure](#3-program-structure)
- [4. Tasks](#4-tasks)
  - [Task 1 - Modelling (15%) 🌎](#task-1---modelling-15-)
    - [Task 1 a) Create Device](#task-1-a-create-device)
    - [Task 1 b) Remove Device](#task-1-b-remove-device)
    - [Task 1 c) Create Satellite](#task-1-c-create-satellite)
    - [Task 1 d) Remove Satellite](#task-1-d-remove-satellite)
    - [Task 1 e) List all device ids](#task-1-e-list-all-device-ids)
    - [Task 1 f) List all satellite ids](#task-1-f-list-all-satellite-ids)
    - [Task 1 g) Add file to device](#task-1-g-add-file-to-device)
    - [Task 1 h) Get device/satellite information](#task-1-h-get-devicesatellite-information)
    - [Task 1 Example](#task-1-example)
  - [Task 2 - Simulation 📡 (25%)](#task-2---simulation--25)
    - [Task 2 a) Run the Simulation](#task-2-a-run-the-simulation)
    - [Task 2 b) List all entities in range](#task-2-b-list-all-entities-in-range)
    - [Task 2 c) File Transferring](#task-2-c-file-transferring)
    - [Task 2 Example](#task-2-example)
  - [Task 3 - Evolution of Requirements (10%) :alien:](#task-3---evolution-of-requirements-10-alien)
    - [a) Moving Devices 📱](#a-moving-devices-)
    - [b) ElephantSatellite 🐘](#b-elephantsatellite-)
...
- [7. Testing and Dryruns 🧪](#7-testing-and-dryruns-)
  - [7.1 Dryrun on CSE](#71-dryrun-on-cse)
...
[**Click here to login and access your personal assignment-i repository**](https://cgi.cse.unsw.edu.au/~cs2511/redirect/?path=COMP2511/24T2/students/_/assignment-i/)
...
<img src="attachments/260898891.png" height="400px"/>
...
<img src="attachments/260898894.png" height="400px"/>
...
<img src="attachments/260898897.png" height="400px"/>
...
<img src="attachments/260898870.png" height="400px"/>
...
The bandwidth is allocated fairly on the sender and receiver no matter the bottleneck from the other side of the transfer. This may lead to "wasted" bandwidth where a file is being allocated more bandwidth than the speed of the transfer.

For instance, let's say a file is being sent between two TeleportingSatellites, `SatelliteA` and `SatelliteB`. `SatelliteA` is sending 4 files (not all of them to `SatelliteB`!). Since a TeleportingSatellite has 10 bytes of sending bandwidth, this means that each file can get 2 bytes of bandwidth. In order to ensure fairness, there are 2 bytes leftover that are now 'wasted'. `SatelliteB` is receiving two files, this means that each file can be allocated `15/2 = 7` bytes. Therefore, the file transfer progresses at at 2 bytes/min, being bottlenecked by the sender. Even though it is bottlenecked at the sender, the file still takes up 7 bytes of bandwidth at the receiver.
...
- `RelaySatellite`
  ...
  - Devices/Satellites cannot transfer files directly to a relay but instead a relay can be automatically used by satellites/devices to send to their real target
    ...
    - An otherwise out of range transfer can be sent through any number of RelaySatellites to reach its destination. For instance, `DeviceA` -> `Relay1` -> `Relay2` -> `SatelliteA`. Of course, this requires that `Relay1` is visible and in range of `DeviceA`, `Relay2` is visible and in range of `Relay1` and `SateliteA` is visible and in range of `Relay2`.
  ...
  - Only travels in the region between `140°` and `190°`
    ...
    - You can either do the radian maths here yourself or use the functions in `app/src/unsw/utils/Angle.java` to do comparisons.
...
<img src="attachments/260898864.png" height="300px"/>
...
[Assignment Sample Implementation](http://cs2511-24t2-assignment-i-sample-env.eba-zsgprhw8.ap-southeast-2.elasticbeanstalk.com/app/)
...
- There are buttons on the top left to refresh the screen (for whatever reason) as well as run simulations for set periods of time
  <img src="attachments/260898885.png" height="350px"/>
...
<table data-layout="full-width" data-local-id="3f2e65c9-3877-4d96-919e-c70776e14fc6" class="confluenceTable">
    <colgroup>
    ...
    <tbody>
        ...
        <tr>
            <td class="confluenceTd">
                <p><code>BlackoutController.java</code></p>
            </td>
            <td class="confluenceTd">
                <p><code>app/src/main/java/unsw/blackout/BlackoutController.java</code></p>
            </td>
            <td class="confluenceTd">
        ...
        <tr>
            ...
            <td class="confluenceTd">
                <p>Runs a server for blackout.</p>
            </td>
            <td class="confluenceTd">
        ...
        <tr>
            <td class="confluenceTd">
                <p><code>MathsHelper.java</code></p>
            </td>
            <td class="confluenceTd">
                <p><code>app/src/main/java/unsw/utils/MathsHelper.java</code></p>
            </td>
            <td class="confluenceTd">
                <p>Contains all the math logic that you'll require.</p>
            </td>
            <td class="confluenceTd">
                <p><strong>No.</strong></p>
            </td>
        ...
        <tr>
            <td class="confluenceTd">
                <p><code>Angle.java</code></p>
            </td>
            <td class="confluenceTd">
                <p><code>app/src/main/java/unsw/utils/Angle.java</code></p>
            </td>
            <td class="confluenceTd">
                <p>Contains an abstraction for angles that lets you easily convert between radians/degrees without
                    having to worry about what 'state' it is currently in.</p>
            </td>
            <td class="confluenceTd">
                <p><strong>No.</strong></p>
            </td>
        ...
        <tr>
            <td class="confluenceTd">
                <p><code>EntityInfoResponse.java</code>and <code>FileInfoResponse.java</code></p>
            </td>
            <td class="confluenceTd">
                <p><code>app/src/main/java/unsw/response/models/EntityInfoResponse.java</code> and <br>
                    <code>app/src/main/java/unsw/response/models/FileInfoResponse.java</code>
                </p>
            </td>
            <td class="confluenceTd">
                <p>Contains the result for certain functions in BlackoutController.</p>
            </td>
            <td class="confluenceTd">
                <p><strong>No.</strong></p>
            </td>
        ...
        <tr>
            <td class="confluenceTd">
                <p><code>Scintilla.java</code> and auxiliary files; <code>Environment.java</code>,
                    <code>PlatformUtils.java</code>, and <code>WebServer.java</code></p>
            </td>
            <td class="confluenceTd">
                <p><code>app/src/main/java/scintilla</code></p>
            </td>
            <td class="confluenceTd">
                <p>Contains a small custom built wrapper around Spark-Java for running a web server. When run it
                    automatically opens a web browser.</p>
            </td>
            <td class="confluenceTd">
                <p><strong>No.</strong></p>
            </td>
        ...
        <tr>
            <td class="confluenceTd">
                <p><code>Task1ExampleTests.java</code></p>
            </td>
            <td class="confluenceTd">
                <p><code>app/src/test/Task1ExampleTests.java</code></p>
            </td>
            <td class="confluenceTd">
                <p>Contains a simple test to get you started with Task 1.</p>
            </td>
            <td class="confluenceTd">
                <p><strong>No</strong>, write your own tests within <code>MyTests.java</code></p>
            </td>
        </tr>
        <tr>
            <td class="confluenceTd">
                <p><code>Task2ExampleTests.java</code></p>
            </td>
            <td class="confluenceTd">
                <p><code>app/src/test/Task2ExampleTests.java</code></p>
            </td>
            <td class="confluenceTd">
                <p>Contains a simple test to get you started with Task 2.</p>
            </td>
            <td class="confluenceTd">
                <p><strong>No</strong>, write your own tests within <code>MyTests.java</code>.</p>
            </td>
...
All methods below exist in the class `app/src/unsw/blackout/BlackoutController.java`
...
> ℹ  NOTE: EntityInfoResponse/FileInfoResponse should only be used as a way of returning information to the frontend or testing framework (via the `getInfo()` method). It should NOT be stored as attributes in your system or used to model entities! Make your own classes for this purpose.
...
You can test your implementations for Task 1 using the simple test provided in the file `app/src/test/Task1ExampleTests.java`. Later you need to add more tests to properly test your implementations.
...
> Checkout `app/src/unsw/blackout/utils/MathHelper.java` there are some very useful functions there that do the vast majority of the work for you, you shouldn't need to figure out any complicated maths.
...
<table data-layout="default" data-local-id="cbfc95e6-d1b2-41e2-bd84-93acb7ece3f5" class="confluenceTable"><colgroup><col style="width: 680.0px;"></colgroup><tbody><tr><th class="confluenceTh"><p>ℹ  For consistency; you should do the movement of satellites before you handle any download/upload for that simulation minute. This also means that bandwidth should be calculated taking into account transfers going out of range that tick. However, this is extension and won't be tested until Task 3. </p></th></tr></tbody></table>

### Task 2 Example

You can test your implementations for Task 2 using the simple test provided in the file `app/src/test/Task2ExampleTests.java`. Later, you will need to add more tests to properly test your implementation. For Task 2 we supply a few different tests just to help you test a variety of cases.
...
<img src="attachments/260898858.png" height="350px"/>
...
<img src="attachments/260898855.png" height="250px"/>
...
<img src="attachments/260898852.png" height="300px"/>
...
- Can receive 20 bytes per minute and can send 20 bytes per minute
...
However, transient files are not held forever; if at any point memory becomes an issue (i.e. there is not enough space for new transfers) it will delete transient files (that aren't currently​ transferring) to make room. Files that are currently transferring or are complete files on the Satellite should not be deleted.

This becomes an optimisation problem known as a 0-1 Knapsack Problem. An Elephant Satellite has a set of transient files, each with a total length (weight) of n bytes and a value being the bytes already transferred. You want to determine which files to keep stored on the Satellite so that the total bytes is less than the available space and to maximise the bytes already transferred.

> View the following link for a description of the required algorithm including pseudocode: [Knapsack Problem](https://en.wikipedia.org/wiki/Knapsack_problem)
...
The example use cases of Tasks 1 and 2 are set up to run against your code in the `app/src/test` directory and are the dryrun for this assignment. There will not be any example tests provided for Task 3 as it is intended to be more challenging.
...
3.  Task 3 is a test of how strong your design is, if you start writing a lot of specific class code in `Blackout` or similar classes (rather than pushing logic 'into' classes) it is probably an indication that you can clean up some aspects of your design.
...
<table data-layout="default" data-local-id="9e7882c4-7572-4216-bf95-9f79a28cb0e8" class="confluenceTable">
    <colgroup>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="confluenceTd">
                <p><strong>Task 1 (15%)</strong></p>
            </td>
            <td class="confluenceTd">
                <p>Your code will be run against a series of autotests to determine the correctness of your solution.
                    You do not need to have completed Tasks 2/3 to receive full marks for Task 1.</p>
            </td>
        </tr>
        <tr>
            <td class="confluenceTd">
                <p><strong>Task 2 (25%)</strong></p>
            </td>
            <td class="confluenceTd">
                <p>Your code will be run against a series of autotests to determine the correctness of your solution.
                    You do not need to have completed Task 3 to receive full marks for Task 2.</p>
            </td>
        </tr>
        <tr>
            <td class="confluenceTd">
        ...
        <tr>
            ...
            <td class="confluenceTd">
                <ul>
                    <li>
                        <p>Have all the appropriate entities been modelled as classes, or is data grouped arbitrarily
                            objects/strings/arrays?</p>
                    </li>
                    <li>
                    ...
                    <li>
                        <p>Are the aggregation and composition relationships and cardinalities shown on the UML logical
                            and appropriate?</p>
                    </li>
                    <li>
        ...
        <tr>
            ...
            <td class="confluenceTd">
                <ul>
                    ...
                    <li>
                        <p>Has functionality been delegated to the appropriate classes and abstracted where needed?</p>
                    </li>
                    <li>
                    ...
                    <li>
                        <p>Is there a lot of logic in the main Blackout class(es) or is it split up?</p>
                    </li>
                    <li>
                        <p>Are the Law of Demeter and Liskov Substitution Principle obeyed?</p>
                    </li>
                </ul>
            </td>
        </tr>
        <tr>
            ...
            <td class="confluenceTd">
                <p>Your code quality and style will be manually marked. See Section 8.</p>
                <p>Half the marks in this section can be achieved by passing the linter in the repository CI.</p>
            </td>
        </tr>
        <tr>
            <td class="confluenceTd">
            ...
            <td class="confluenceTd">
                <ul>
                    ...
                    <li>
                        <p>Do the blog posts contain reflection and areas for improvement?</p>
                    </li>
                    <li>
                        <p>Do the blog posts demonstrate an understanding of OO design concepts and their application?
                        </p>
                    </li>
                </ul>
            </td>
...
Assignment authored by Braedon Wooding, Nick Patrikeos, Carl Buchanan and Ashesh Mahidadia.
